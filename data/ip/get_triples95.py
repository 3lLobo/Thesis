...
with open('data/ip/{}_conf95_all.txt'.format(ip_name), 'w+') as f:
    for i_dim in range(10):
        ip_95 = all_dict['interpolations.confidence95.text.{}'.format(i_dim)]
        f.writelines('\\hline \n')
        f.writelines(f'\\rowcolor[HTML]{{EFEFEF}} \n')
        f.writelines(f'\\textsc{{$i_{{d_z}}=' + str(i_dim+1) + '$}\\\\ \\hline \n')
        for n in range(10):
            triple2write = '\\texttt{'+str(ip_95[n]).replace('_','\\_').replace('\'','').replace(', ','] [')+'}\\\\\n'
            if './' in triple2write:
                tri_1, tri_2 = triple2write.split('./', 1)
                triple2write = tri_1 + ']' + tri_2.split(']', 1)[1]
            f.writelines(triple2write)
