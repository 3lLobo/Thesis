...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $A_{term1} = A$.unsqueeze(-1)$^T (\tilde{A} \odot (\tilde{A}  \tilde{A}^T))$.unsqueeze(-1) \Comment{Dot product over the last (empty) dimension}
        \State $S_r = E_{term1} \odot A_{term1}$
        \Statex \textbf{Second term:} $[i=j \wedge a=b]$
        \State $A_{term2} =$ ones\_like(diag($\tilde{A}$))$^T $ diag($\tilde{A}$)
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \For{$iteration=1,2,\dots$}
            \State $S_{max} =$ sum(max($S_r \odot X^*$.unsqueeze($[1,1]$))) \Comment{Sum and max over the last dimension. Unsqueeze two times on the second dimension}
            \State $X^* = X^* \odot S_e + S_{max}$
            \State $X^* = X^* /$ frobnorm($X^*$)
        \EndFor
...
Our main experiment will be link prediction. This will be a proof of concept rather than an attempt to set the state of the art. The results will let us draw conclusions on the impact and function of different parameters.
