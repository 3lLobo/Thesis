...
The max-pooling graph matching algorithm returns a symmetric affinity matrix for all nodesV

The resulting similarity matrix gives us $X*$ which is continuous and therefore useless for precise graph matching. To transform is to a discrete $X$ we use the hungarian algorithm (GPU-accelerated Hungarian algorithms for the Linear
Assignment Problem)
